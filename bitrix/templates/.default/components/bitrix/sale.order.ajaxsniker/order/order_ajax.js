...
*/
		...
		clickNextAction: function(event)
		{
			var target = event.target || event.srcElement,
				...
				allSections, titleNode, editStep, isCheck = true;
			...
			switch (actionSection.id) {
				case 'bx-soa-properties':

					$('#'+actionSection.id).find('[data-required="true"]').each(function() {
						if ($(this).find('.form-control').val() == '') {
							var label = $(this).find('.bx-soa-custom-label');
                            				$(this).addClass('has-error');
				                        if (!$(this).find('.bx-soa-tooltip').length)
                            					label.after('<div id="tooltip-'+$(this).find('.form-control').attr('id')+'" class="bx-soa-tooltip bx-soa-tooltip-static bx-soa-tooltip-danger tooltip top" data-state="opened" style="opacity: 1; display: block;"><div class="tooltip-arrow"></div><div class="tooltip-inner">заполните поле</div></div>');
				                        isCheck = false;
						} else {
							if ($(this).hasClass('has-error')) {
					                        isCheck = false;
							};
                            if($('[data-code=PHONE]').find('.form-control').val()!=''){
                                $('[data-code=PHONE]').removeClass('has-error');
                                $('[data-code=PHONE]').find('.bx-soa-tooltip-danger').hide();
                            } else {
                            $('[data-code=PHONE]').addClass('has-error');
                                $('[data-code=PHONE]').find('.bx-soa-tooltip-danger').show();   
                            };
						}    
					});
					...
					$('#'+actionSection.id).find('[data-required="true"]').each(function() {
						if ($(this).find('.form-control').val() == '') {
                            ...
                            $('[data-code=PHONE]').addClass('has-error');
                               ...
                               if($('.bx-soa-location-input-container').hasClass('has-error')!=false){
       isCheck = false; 
       };
					});
				...
				case 'bx-soa-delivery':
					if($('#'+actionSection.id).find('[name="PSEUDO_DELIVERY"]:checked').length == 0){
	                                        $('#'+actionSection.id).find('.alert-danger').html('Р’С‹Р±РµСЂРёС‚Рµ СЃРїРѕСЃРѕР± РґРѕСЃС‚Р°РІРєРё').show();
			                        isCheck = false;
					}else{
						if($('#'+actionSection.id).find('[name="PSEUDO_DELIVERY"]:checked').val() == 183){		
						}
					}
					break;
			}
			...
			if (isCheck) {
				$('[data-title-id="'+actionSection.id+'"] a').addClass('triggerable');
                $('[data-title-id="'+section.next.id+'"]').addClass('active');
                $('[data-title-id="'+section.next.id+'"]').siblings().removeClass('active');
                this.fade(actionSection, section.next);
                this.show(section.next);
                if (section.next.id == 'bx-soa-basket') {
                    document.getElementById('final-contacts-data').innerHTML = $('[data-code="FIO"]').find('input').val() + '<br>' +
                        $('[data-code="EMAIL"]').find('input').val() + '<br>' + $('[data-code="PHONE"]').find('input').val();
                    document.getElementById('final-address-data').innerHTML = $('#bx-soa-region-hidden').find('.bx-ui-sls-route').val() + ', ' +
                        $('[data-code="ZIP"]').find('input').val() + ', ' + $('[data-code="ADDRESS"]').find('.form-control').val();
                    document.getElementById('final-delivery-data').innerHTML = $('#bx-soa-delivery-hidden .bx-selected .bx-soa-pp-company-smalltitle').text() +
                        ' ' + $('#bx-soa-delivery-hidden .bx-selected .bx-soa-pp-delivery-cost').html();
                    document.getElementById('final-paysystem-data').innerHTML = $('#bx-soa-paysystem-hidden .bx-selected .bx-soa-pp-company-smalltitle').text();
                    document.getElementById('final-sum-data').innerHTML = $('#bx-soa-total .bx-soa-cart-total-line-total .bx-soa-cart-d').html();
                    document.getElementById('bx-soa-orderSave').style.display = 'block';
				}
            }
			return BX.PreventDefault(event);
		},
...
onCheckoutOption(5);
			...
			if (node.getAttribute('data-visited') === 'false') {
                ...
                $('#'+node.id).find('.has-error').each(function() {
                	$(this).find('.bx-soa-tooltip').remove();
		...
		/*onCheckoutOption: function(step){
		},*/
		...
		getBlockFooter: function(node)
		{
			...
			if (currentSection && currentSection.id.indexOf(firstSection.id) == '-1')
			{
				buttons.push(
					BX.create('A', {
						props: {
							href: '',
							className: 'pull-left btn btn-prev'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: '', className: 'pull-right btn btn-next'},
...
setTimeout(function () {
var t = $('.shipping_methods td').length;
if(t<=0){
    $('#bx-soa-region').find('.bx-soa-more-btn').remove();
    $('#bx-soa-region').find('.bx-soa-more').html('<div class="errorCart">Доставка в Москву не осуществляется</div>');
}    
},100);
        ...
        // РџРµСЂРµРјРµС‰РµРЅРёРµ РїРѕР»РµР№
        propMove: function ()
        {
			if ($('[data-code="ADDRESS"]').length && $('[data-code="ZIP"]').length) {
                $('[data-code="ZIP"]').after('<div data-code-double="ADDRESS-CLONE"></div>');
                $('[data-code="ADDRESS"]').appendTo($('[data-code-double="ADDRESS-CLONE"]'));
			}
			if ($('[data-code="ORDER_DESCRIPTION"]').length && !$('#bx-soa-orderSave').find('[data-code="ORDER_DESCRIPTION"]').length) {
                $('[data-code="ORDER_DESCRIPTION"]').appendTo($('#bx-soa-orderSave').find('.pseudo-comment'));
			}
        },

        pseudoDelivery: function()
        {
            var data = '<table class="shipping_methods"><tbody>';
            for (var k = 0; k < this.deliveryPagination.currentPage.length; k++)
            {
            	...
            	data = data + '<tr class="highlight">\
                    <td>\
                    <input class="img-ckeckbox" '+checked+' type="radio" name="PSEUDO_DELIVERY" value="'+this.deliveryPagination.currentPage[k].ID+'" id="pseudo-delivery-'+this.deliveryPagination.currentPage[k].ID+'">\
                	<label for="pseudo-delivery-'+this.deliveryPagination.currentPage[k].ID+'">\
                	<i class="checbox-img-label"></i>\
                	<span class="bold600">'+this.deliveryPagination.currentPage[k].NAME+'</span><br>\
            		'+this.deliveryPagination.currentPage[k].DESCRIPTION+'</label><div id="de-'+this.deliveryPagination.currentPage[k].ID+'"></div></td>\
            		<td class="shipping_method_price">'+((this.deliveryPagination.currentPage[k].PRICE == undefined)?'':this.deliveryPagination.currentPage[k].PRICE+' <i class="fa fa-rub"></i>')+'</td>\
                	</tr>';
	    }
            data = data + '</tbody></table>';
            if (!$('#bx-soa-delivery').find('.pseudo-delivery').length){
                $('#bx-soa-delivery .bx-soa-section-content').before('<div class="pseudo-delivery">'+data+'</div>');
	    } else {
                $('#bx-soa-delivery').find('.pseudo-delivery').html(data);
	    }

            var selectedDelivery = this.getSelectedDelivery();

            if (selectedDelivery.STORE && selectedDelivery.STORE.length > 0 && this.pickUpPagination.currentPage && this.pickUpPagination.currentPage.length > 0) {
				$('[data-title-id="bx-soa-pickup"]').show();
            } else {
                $('[data-title-id="bx-soa-pickup"]').hide();
			}

			if (selectedDelivery.NAME == 'PickPoint' && $('[data-code="ADDRESS"] .form-control').val() == '') {
            	$('#bx-soa-delivery .bx-soa-more-btn .btn-next').attr('disabled', 'disabled');
			}
		},

        pseudoPayment: function()
        {
            var data = '<table class="payment_methods"><tbody>';
            for (var k = 0; k < this.paySystemPagination.currentPage.length; k++)
            {
                var checked = '';
                if (this.paySystemPagination.currentPage[k].CHECKED == 'Y') {
                    checked = 'checked="checked"';
                }
                data = data + '<tr class="highlight">\
                    <td>\
                    <input class="img-ckeckbox" '+checked+' type="radio" name="PSEUDO_PAYMENT" value="'+this.paySystemPagination.currentPage[k].ID+'" id="pseudo-payment-'+this.paySystemPagination.currentPage[k].ID+'">\
                	<label for="pseudo-payment-'+this.paySystemPagination.currentPage[k].ID+'">\
                	<i class="checbox-img-label"></i>\
                	<span class="bold600">'+this.paySystemPagination.currentPage[k].NAME+'</span><br></label></td>\
           			</tr>';
            }
            data = data + '</tbody></table>';
            if (!$('#bx-soa-paysystem').find('.pseudo-paysystem').length){
                $('#bx-soa-paysystem .bx-soa-section-content').before('<div class="pseudo-paysystem">'+data+'</div>');
            } else {
                $('#bx-soa-paysystem').find('.pseudo-paysystem').html(data);
            }
        },
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
                ...
                propsItemNode.setAttribute('data-code', 'ZIP');
if(zipProperty.REQUIRED == 'Y'){
 propsItemNode.setAttribute('data-required', 'true');   
}
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
                ...
                // РґРѕР±Р°РІР»СЏРµРј РєРѕРґ РґР»СЏ Р±Р»РѕРєР° СЃРѕ СЃРІРѕР№СЃС‚РІРѕРј
                var props = property.getSettings();
                propsItemNode.setAttribute('data-code', props.CODE);
                if (property.isRequired())
                    propsItemNode.setAttribute('data-required', 'true');
		...
		alterProperty: function(settings, propContainer)
		{
			...
			if (textNode)
			{
				...
				if (settings.IS_PHONE == 'Y')			    
		                    	textNode.setAttribute('autocomplete', 'phone');
		...
		editPropsComment: function(propsNode)
		{
			...
			div = BX.create('DIV', {
				...
				attrs: {'data-code': 'ORDER_DESCRIPTION'},
				children: [input]
			});
		...
		editTotalBlock: function()
		{
			...
			this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_SUMMARY'), priceHtml, params, 'bx-soa-cart-sum-product'));
/*
...
*/
			...
			{
				if(curDelivery){ //FM add 16.03.2018
					this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params, 'bx-soa-cart-sum-delivery'));
				}
		...
		createTotalUnit: function(name, value, params, classNamePrice)
		{
			...
			return BX.create('DIV', {
				...
				children: [
					...
					BX.create('SPAN', {
						props: {
							className: 'bx-soa-cart-d ' + classNamePrice  + (!!params.total && this.options.totalPriceChanged ? ' bx-soa-changeCostSign' : '')
						},
						children: totalValue
					})
				]
...
$(document).on('change', '[name="PSEUDO_DELIVERY"]', function(){
...
$(document).on('change', '[name="PSEUDO_PAYMENT"]', function(){
	$('#ID_PAY_SYSTEM_ID_'+$(this).val()).click();
});
$(document).on('click', '[data-prev-button="true"]', function(){
	document.getElementById('bx-soa-orderSave').style.display = 'none';
	document.getElementById('bx-soa-basket').querySelector('.btn-prev').click();
	return false;
});
$(window).load(function(){
	if($('[name="ORDER_PROP_3"]').length > 0){
		$('[name="ORDER_PROP_3"]').mask('+7 (999) 999-99-99',{placeholder:'+7 (___) ___-__-__'});
	}
	$('[name="ORDER_PROP_3"]').focusout(function() {
		setTimeout(function(){   
			if($('[data-code=PHONE]').find('.form-control').val().length>=2){
				$('[data-code=PHONE]').removeClass('has-error');
				$('[data-code=PHONE]').find('.bx-soa-tooltip-danger').hide();
			} else {
				$('[data-code=PHONE]').addClass('has-error');
	...
	$('a.pull-lea-ft.btn.btn-prev').on('click',function(){
		if($('#bx-soa-properties').hasClass('bx-selected')!=false){
