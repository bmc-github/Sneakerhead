...
use Bitrix\Main, Bitrix\Sale, Bitrix\Main\Localization\Loc, Bitrix\Main\Page\Asset;
...
if (!empty($arResult['ERRORS']['FATAL'])) {
    ...
    if ($arParams['AUTH_FORM_IN_TEMPLATE'] && isset($arResult['ERRORS']['FATAL'][$component::
        E_NOT_AUTHORIZED])) {
...
?>
    ...
    if ($_REQUEST["filter_history"] !== 'Y') {
        ...
        foreach ($arResult['ORDERS'] as $key => $order) {
            ...
            if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $arResult['SORT_TYPE'] ==
                'STATUS') {
...
?>
			<div class="order_item">
				<div class="order_item_info">
					<p class="order_item_info_title">
						<a >
							<span class="order_item_info_title_orderid">
								<?= Loc::getMessage('SPOL_TPL_ORDER') ?>
                                                               
								<?= Loc::getMessage('SPOL_TPL_NUMBER_SIGN') . $order['ORDER']['ACCOUNT_NUMBER'] ?>
							</span>
						</a>
						<span class="order_item_info_title_date">
							<?= Loc::getMessage('SPOL_TPL_FROM_DATE') ?>
							<?= $order['ORDER']['DATE_INSERT']->format('Y-m-d H:i:s'
/*$arParams['ACTIVE_DATE_FORMAT']*/ ) ?>
            ...
            $statusx = $arStatus['NAME'];


            $showDelimeter = false;
            if ($order["ORDER"]["PAYED"] == 'N' && $order["ORDER"]["PAY_SYSTEM_ID"] == 10 &&
                ($order['ORDER']['STATUS_ID'] == "SZ")) { ?>  
                        <style>
a.goPayment.gP {
    background: #F20113;
    color: white;
...
<script>
$('.gP').click (function clicknews () {
$(this).html('Ожидайте...');
var toLoad = $(this).attr('href')+' #PayU_Form';
$('.vst').load(toLoad,'');

setInterval(function (){$('body').find('[type="submit"].goPayment').parents('form').submit()},400);
return false;
});
</script>                
                        <a class="goPayment gP" href="/checkout/?ORDER_ID=<?= $order["ORDER"]['ID'] ?>&PAYMENT=Y">Перейти к онлайн оплате</a>
                        <div class="vst" style="display: none;"></div>
                        <? }
...
?>
                ...
                ?>
                <p class="sale-order-list-shipment-item">
            ...
            <?
						...
						<div class="sale-order-list-inner-title-line">
								<span class="sale-order-list-inner-title-line-item">Детали заказа</span>
								<span class="sale-order-list-inner-title-line-border"></span>
						</div>
            ...
            $orderz = Sale\Order::load($order['ORDER']['ID']);
            $basket = $orderz->getBasket();
...
?>
                                         ...
                                         <?
            if (count($order['BASKET']) == 0) {
                foreach ($basket as $basketItem) {

                    $temp = GetIBlockElement($basketItem->getField('PRODUCT_ID'));
                
                    $renderImage = CFile::ResizeImageGet($temp['PREVIEW_PICTURE'], array("width" =>
                            120, "height" => 120));
                    $basketPropertyCollection = $basketItem->getPropertyCollection();
                    $vals = $basketPropertyCollection->getPropertyValues();
                    $size = $vals["SIZE"]["VALUE"];

                    if (!($renderImage['src']) || $renderImage['src'] == '') {


                        $res = CIBlockElement::GetByID($basketItem->getField('PRODUCT_ID'));
                        if ($ar_res = $res->GetNext()) {

                            $renderImage = CFile::ResizeImageGet($ar_res['PREVIEW_PICTURE'], array("width" =>
                                    120, "height" => 120));
                    ...
                    if (!($renderImage['src']) || $renderImage['src'] == '') {
                           ...
                           $renderImage['src'] = '/upload/noi.jpg';
                    }
...
?>
                                         	...
                                         	<table class="order_history_products_item">
													<tr>
															<td class="order_history_products_item_image_wr">
                                                            
                                                            <img src="<? echo $renderImage['src']; ?>" style="width: 120px;height:120px;">
                                                            </td>
                                                            
                                                            
                                                            	<td class="order_history_products_item_name_wr">
                                                            <?= $basketItem->
getField('NAME'); ?>
                                                            </td>
                                                            
                                                            
                                                            	<td class="order_history_products_item_options_wr">
																<div class="order_history_products_item_option">
																	
																Размер: <?= $size; ?>
																</div>
																
															</td>
															<td class="order_history_products_item_price_wr">
																<span><?= number_format($basketItem->getPrice(), 2, '.', ' ') ?> <i class="fa fa-rub"></i></span>
															</td>
															<td class="order_history_products_item_options_wr">
																<div class="order_history_products_item_option">
													...
													<tr>
															<td class="order_history_products_item_image_wr">
																<a href="<?= $basketItem['PRODUCT']['DETAIL_PAGE_URL'] ?>">
																	<?
                if (strlen($basketItem['PRODUCT']['PREVIEW_PICTURE'])) {
                    $imageSrc = $basketItem['PRODUCT']['PREVIEW_PICTURE'];
                } else {
                    $imageSrc = $this->GetFolder() . '/images/no_photo.png';
                }
?>
																	<img class="sale-order-detail-order-item-imgcontainer" src="<?= $imageSrc ?>" alt="" style="width: 120px;height:120px;" />
																</a>
															</td>
															<td class="order_history_products_item_name_wr">
																<div class="order_history_products_item_name_category">
																	<a href="<?= $basketItem['CATEGORY']['DETAIL_PAGE_URL'] ?>">
																		<?= htmlspecialcharsbx($basketItem['CATEGORY']['NAME']) ?>
																	</a>                                               																												
																</div>
																<div>
																	<a class="order_history_products_item_manufacturer_name" href="<?= $basketItem['BRAND']['DETAIL_PAGE_URL'] ?>">
																		<?= htmlspecialcharsbx($basketItem['BRAND']['NAME']) ?>
																	</a>
																	<a class="order_history_products_item_name" href="<?= $basketItem['PRODUCT']['DETAIL_PAGE_URL'] ?>">
																		<?= htmlspecialcharsbx($basketItem['PRODUCT']['NAME']) ?>
																	</a>
																</div>
															</td>
															<td class="order_history_products_item_options_wr">
																<div class="order_history_products_item_option">
																	Размер:
																	<?= htmlspecialcharsbx($basketItem['PRODUCT']['SIZE']) ?>
																</div>
																
															</td>
															<td class="order_history_products_item_price_wr">
																<span><?= number_format($basketItem['PRODUCT']['PRICE'], 2, '.',
...
?>
						...
						<p class="fontUC" style="font-size:20px; font-weight:300">
							Итого: <span class="bold700U"><?= $order['ORDER']['FORMATED_PRICE'] ?></span>
						</p>
						<? /*<div class="row sale-order-list-inner-row">
            <div class="sale-order-list-top-border"></div>
...
?>
        ...
        foreach ($arResult['ORDERS'] as $key => $order) {
            if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $_REQUEST["show_canceled"]
                !== 'Y') {
                $orderHeaderStatus = $order['ORDER']['STATUS_ID'];
?>
				<h1 class="sale-order-title">
