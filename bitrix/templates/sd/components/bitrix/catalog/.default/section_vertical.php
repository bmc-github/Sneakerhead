        ...
        <div class="col-xs-12">
	      ...
	      <td style="text-align:right;"><?echo '<!--noindex-->';?>
	        <div class="product-filter">
    		      ...
    		      <option value="?sort=date&order=desc"<?if($param=='sort=date&order=desc') echo ' selected';?>><?=GetMessage('H_SORT')?></option>
    		      <option value="?sort=price&order=asc"<?if($param=='sort=price&order=asc') echo ' selected';?>><?=GetMessage('H_PRICE_ASC')?></option>
    		      <option value="?sort=price&order=desc"<?if($param=='sort=price&order=desc') echo ' selected';?>><?=GetMessage('H_PRICE_DESC')?></option>
    		    </select>
    	          </div>
     	        </div><?echo '<!--/noindex-->';?>
     	      </td>
              ...
              if (\Bitrix\Main\Loader::includeModule('iblock')) {
                  for ($i = 0; $i < count($uri_parts); $i++) {
                      $rsResult = \Bitrix\Iblock\SectionTable::getList([
                          'select' => ['ID'],
                          'filter' => ['=IBLOCK_ID' => $arParams['IBLOCK_ID'], '=CODE' => $uri_parts[$i]],
                      ]);
...
<?endif?>
        <div class="<?=(($isFilter || $isSidebar) ? "col-xs-9" : "col-xs-12")?>">
	<?/*<!--<div class="row">
		<div class="col-xs-12">
			<?
...
//pr($arResult);
			...
			$intSectionID = $APPLICATION->IncludeComponent(
				...
				array(
					...
					"SECTION_URL" => $arResult["FOLDER"].$arResult["URL_TEMPLATES"]["section"],
					"DETAIL_URL" => $arResult["FOLDER"].$arResult["URL_TEMPLATES"]["element"],
					"CANONICAL_URL" => $arParams["DETAIL_SET_CANONICAL_URL"],
