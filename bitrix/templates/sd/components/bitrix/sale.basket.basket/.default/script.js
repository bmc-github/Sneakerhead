...
function updateBasketTable(basketItemId, res)
{
	...
	if (!!res.BASKET_DATA)
	{
		...
		if ($('#bx-soa-total').length) {
			$('.bx-soa-cart-sum-product').html(res['BASKET_DATA']['allSum_wVAT_FORMATED'].replace(/\s/g, '&nbsp;') + '<i class="fa fa-rub"></i>');
            $('.bx-soa-cart-total-line-total .bx-soa-cart-d').html(res['BASKET_DATA']['allSum_wVAT_FORMATED'].replace(/\s/g, '&nbsp;') + '<i class="fa fa-rub"></i>');
		}

		if (BX('allSum_wVAT_FORMATED'))
			BX('allSum_wVAT_FORMATED').innerHTML = res['BASKET_DATA']['allSum_wVAT_FORMATED'].replace(/\s/g, '&nbsp;') + '<i class="fa fa-rub"></i>';

		if (BX('allVATSum_FORMATED'))
			BX('allVATSum_FORMATED').innerHTML = res['BASKET_DATA']['allVATSum_FORMATED'].replace(/\s/g, '&nbsp;') + '<i class="fa fa-rub"></i>';

		if (BX('allSum_FORMATED'))
			BX('allSum_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;') + '<i class="fa fa-rub"></i>';
		...
		if (BX('PRICE_WITHOUT_DISCOUNT'))
		{
			...
			BX.style(BX('PRICE_WITHOUT_DISCOUNT').parentNode, 'display', (showPriceWithoutDiscount ? 'table-row' : 'none'));
                ...
                $('.header-cart .cart-total').html($cart_qty);
