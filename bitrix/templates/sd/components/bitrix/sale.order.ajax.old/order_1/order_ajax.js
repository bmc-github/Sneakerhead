	...
	BX.Sale.OrderAjaxComponent = {
			...
			switch (actionSection.id) {
						...
						if ($(this).find('.form-control').val() == '') {
							var label = $(this).find('.bx-soa-custom-label');
				                        ...
				                        if (!$(this).find('.bx-soa-tooltip').length)
                            					label.after('<div id="tooltip-'+$(this).find('.form-control').attr('id')+'" class="bx-soa-tooltip bx-soa-tooltip-static bx-soa-tooltip-danger tooltip top" data-state="opened" style="opacity: 1; display: block;"><div class="tooltip-arrow"></div><div class="tooltip-inner">Поле обязательно для заполнения</div></div>');
				                        isCheck = false;
						} else {
				...
				case 'bx-soa-delivery':
					if($('#'+actionSection.id).find('[name="PSEUDO_DELIVERY"]:checked').length == 0){
	                                        $('#'+actionSection.id).find('.alert-danger').html('Выберите способ доставки').show();
			                        isCheck = false;
					}else{
        ...
        pseudoDelivery: function()
        {
            ...
            for (var k = 0; k < this.deliveryPagination.currentPage.length; k++)
            {
            	...
            	data = data + '<tr class="highlight">\
            		...
            		'+this.deliveryPagination.currentPage[k].DESCRIPTION+'</label></td>\
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
                ...
                // добавляем код для блока со свойством
