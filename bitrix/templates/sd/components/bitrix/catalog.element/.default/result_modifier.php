...
$stock_status = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 18, 'ID' =>$arResult['PROPERTIES']['STOCK_STATUS']['VALUE']), false, false, array('PREVIEW_TEXT'))->GetNext();
...
$rs = CIBlockElement::GetList(array("SORT" => "ASC"), array(
    'IBLOCK_ID' => 2,
    'ID' => $arResult['PROPERTIES']['RELATED_PRODUCTS']['VALUE'],
    'ACTIVE' => 'Y'), false, false, array(
    'ID',
    'NAME',
    'DETAIL_PAGE_URL',
    'PROPERTY_ARTNUMBER',
...
foreach ($arResult['OFFERS'] as $i => $offer) {
    ...
    $rs = CCatalogStoreProduct::GetList(array('ID' => 'ASC'), array('PRODUCT_ID' =>
            $offer['ID']), false, false, array(
        'ID',
        'STORE_ID',
        'STORE_NAME',
        'STORE_ADDR',
    ...
    while ($store = $rs->GetNext()) {
        if ($store['AMOUNT'] > 0) {
            $arResult['STORES_MAP'][$offer['ID']][] = array(
                'ID' => $store["STORE_ID"],
                'NAME' => $store['STORE_NAME'],
                'ADDRESS' => $store['STORE_ADDR'],
                'GEO_LAT' => $store['STORE_GPS_N'],
                'GEO_LONG' => $store['STORE_GPS_S'],
                'AMOUNT' => $store['AMOUNT']);
            $arResult['STORES'][$offer['ID']][] = $store['STORE_NAME'];
        }
    }

    if ($arResult['PROPERTIES']['SIZECHART']['VALUE']!=''){
        $brand['PROPERTY_SIZE_CHART_VALUE']=$arResult['PROPERTIES']['SIZECHART']['VALUE'];
    }    
...
if ($haveOffers)
    $actualItem = isset($arResult['OFFERS'][$arResult['OFFERS_SELECTED']]) ? $arResult['OFFERS'][$arResult['OFFERS_SELECTED']] :
...
if ($arResult['DISPLAY_PROPERTIES']['GENDER']['DISPLAY_VALUE']) {
    ...
    switch ($cat_) {
        ...
        default:
            {
                if (mb_substr($gender_, 0, 1, "UTF-8") == 'М')
                    $adg = 'ой';
                else
                    $adg = 'ий';
            }
...
if ($category['IBLOCK_SECTION_ID'] == 11 || $category['IBLOCK_SECTION_ID'] == 12 || $category['ID'] == 11 || $category['ID'] == 12) {
    $arResult['META']['TITLE'] = ($color ? $color . ' ' : '') . ($gender ? ToLower($gender) . ' ' : '') . ToLower($cat) . ' ' . $arResult['NAME'] . ' от ' . $brand['NAME'] .
        ' (' . $sku . ') по цене ' . $price['RATIO_PRICE'] . ' рублей';
    ...
    $arResult['META']['TITLE'] = 'Купить ' . ToLower(($color ? $color . ' ' : '') . ($gender ? $gender . ' ' : '') . $cat) . ' ' . $arResult['NAME'] . ' от ' . $brand['NAME'] .
        ' (' . $sku . ') по цене ' . $price['RATIO_PRICE'] . ' рублей';
    $arResult['META']['KEYWORDS'] = ToLower(($color ? $color . ' ' : '') . ($gender ? $gender . ' ' : '') . $cat) . ' ' . $arResult['NAME'] . ' ' . $brand['NAME'] .
        ' ' . $sku . ' цена купить интернет магазин';
    $arResult['META']['DESCRIPTION'] = 'Купить ' . ToLower(($color ? $color . ' ' : '') . ($gender ? $gender . ' ' : '') . $cat) . ' ' . $arResult['NAME'] . ' от ' .
        $brand['NAME'] . ' (' . $sku . ') по лучшей цене с фото, описанием и отзывами только в мультибрендовом интернет магазине Sneakerhead.';
}
$cp = $this->__component;
if (is_object($cp))
