...
$name = $arResult['NAME'];
$arResult['DETAIL_TEXT'] = preg_replace_callback('/<div class="t001__title t-title t-title_xl" field="title">(.*)<\/div>/isU', function($matches) use (&$name){
	if(str_replace(array('<br>','<br />'),' ',$matches[1]) == $name){
