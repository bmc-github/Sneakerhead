      ...
      if($USER->IsAuthorized()){
        if($_SESSION['moscow']){
          $msg = sprintf('<p>Номер вашего заказа %s.</p><p>В данный момент заказ успешно размещен и собирается.</p><p>В течение часа с Вами свяжется оператор, чтобы уточнить детали доставки.</p>', $orderId);
     	  ...
     	  $msg = sprintf('<p>Номер вашего заказа %s.</p><p>Мы отправили на вашу почту письмо со всей информацией.</p><p>В данный момент заказ успешно размещен и обрабатывается.</p><p>Мы проверим наличие вашего заказа на складе и сообщим результат дополнительным письмом.</p>', $orderId);
        }
      }else{
	$msg = sprintf('<p>Номер вашего заказа %s.</p><p>В данный момент заказ успешно размещен и собирается.</p><p>В течение часа с Вами свяжется оператор, чтобы уточнить детали доставки.</p>', $orderId);
      }  
      echo $msg . $script;//echo json_encode(array('order_id'=>$orderId,'msg'=>$msg));
      CSaleBasket::DeleteAll(CSaleBasket::GetBasketUserID());
    }else{
      echo 'Ошибка оформления';//echo json_encode(array('order_id'=>$orderId,'msg'=>'Ошибка оформления'));
    }
?>