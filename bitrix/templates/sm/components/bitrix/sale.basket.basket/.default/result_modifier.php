...
foreach($arResult["GRID"]["ROWS"] as $key=>$arItem){
  ...
  $product = CIBlockElement::GetList(array(), array('IBLOCK_ID'=>2,'ID'=>$sku['ID']), false, false, array('IBLOCK_ID','ID','IBLOCK_SECTION_ID','XML_ID','NAME','PROPERTY_BRAND','PROPERTY_SALE','PROPERTY_SPECIAL_PRICE','PROPERTY_SPECIAL_DATE'))->GetNext();//CIBlockElement::GetByID($sku['ID'])->GetNext();  
