...
function myOnSaleComponentOrderResultPrepared($order, &$arUserResult, $request,
    ...
    //($ID, $arFields)
{
	mail('dimm4ik@yandex.ru','test',print_r($arParams,true).print_r($arResult,true).print_r($arPaySystemServiceAll,true));
    ...
    foreach ($basketItems as $Item) {
        ...
        $product = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 2, 'ID' => $pid), false, false,
            array(
            'IBLOCK_ID',
            'ID',
            'PROPERTY_STOCK_STATUS'))->GetNext();
        $arrayP[] = $product["PROPERTY_STOCK_STATUS_VALUE"];
		if($product["PROPERTY_STOCK_STATUS_VALUE"] == '22730'){
			$noMoscow = true;
		}
    }

	if($Location == '0000073738' && 			$noMoscow){
		unset($arPaySystemServiceAll);
		unset($arDeliveryServiceAll);
	}
	/*
...
{
    //$prM = \Bitrix\Sale\Order::load($orderId);
    //$prM->setfield('ACCOUNT_NUMBER', 3);
    //$result = $prM->save();
	...
	if((($paymentSelected == 1 || $paymentSelected ==2) || ($price == $paydSum)) && $pr->getField('ALLOW_DELIVERY') == 'N'){//->getPaymentSystemId()== 'N'){
                    ...
                    if ($el->Add($arFields)) {
                        ...
                        foreach ($storeRAZ as $onestore) {
                            for ($i = 1; $i <= (float)$onestore["col"]; $i++) {
                                ...
                                curl_setopt($ch, CURLOPT_URL, 'http://test.sneakerhead.ru/logistic/size.php');
                                curl_setopt($ch, CURLOPT_REFERER, 'http://test.sneakerhead.ru/logistic/size.php');
    ...
    if (!$OrderY['ID']) {
        ...
        $propertyCollectionZ = $prZ->getPropertyCollection();
        //Собираем все поля
        //orderNumber - ID заказа
        $dom_xmlV = new DomDocument("1.0", "utf-8");
        ...
        $orderV->appendChild($orderNumberV);
        //FIO - имя клиента + телефон
        ...
        //$DATE_UPDATE = str_replace('.', '-', $DATE_UPDATE);
        ...
        while ($itZ = $rbZ->GetNext()) {
            ...
            if ($offerZ) {
                //1с код
                $CODE_1C = $offerZ["PROPERTY_C_CODE_VALUE"];
                //Создаем элемент code1c
                $code1cV = $dom_xmlV->createElement("code1c", $CODE_1C);
                $itemV->appendChild($code1cV);
                //Артикул
                $ARTICLE = $offerZ["PROPERTY_ARTNUMBER_T_VALUE"];
            ...
            $itemV->appendChild($discount_percentV);
            //Величина скидки
            ...
            $itemV->appendChild($discount_summV);
            //Колличество определенной единицы
            $QU = $itZ["QUANTITY"];
            //Создаем элемент quantity
            $quantityV = $dom_xmlV->createElement("quantity", $QU);
            $itemV->appendChild($quantityV);
            //дата обновления
            $DATE_UPDATE = $DATE_UPDATE;
            //Создаем элемент change_time
            ...
            $pathV = 'ftp://sneake01_ftp:6BjvOObrXT@ftp.sneake01.nichost.ru/fromHC/bitrix/Orders/snrk_' .
...
function My_OnOrderSave2($orderId)
{
    ...
    $OrderY = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 19,
    "PROPERTY_ORDER" => $order_id), false, false, array("ID", "IBLOCK_ID"))->GetNext();
    ...
    ), false, false, array("ID", "IBLOCK_ID","NAME","PROPERTY_C_CODE","PROPERTY_101","PROPERTY_89","PROPERTY_93","PROPERTY_95","PROPERTY_91","PROPERTY_96"));
    $item = array();
    $item_values = array();
    ...
    'PRODUCT_ID' => $key,
    'STORE_ID' => $STORE_ID), false, false, array('AMOUNT', 'STORE_ID'))->GetNext();
    if (isset($rsStore['AMOUNT']) && $rsStore['AMOUNT'] != false && $rsStore['AMOUNT'] >= 1) {
