...
class RetailCrmHistory
{
    ...
    public static function customerHistory()
    {
        ...
        if ($historyStart && $historyStart > 0) {
			...
			//$historyFilter['externalId'] = 47427;
        }
...
$optionsDelivTypes['pickpoint'] = 183;
        ...
        if ($historyStart && $historyStart > 0) {
			$historyFilter['sinceId'] = $historyStart;
			//$historyFilter['orderExternalId'] = 71050;
        }      
                ...
                if (isset($order['externalId'])) {
                    ...
                    if ($optionsOrderProps[$personType]) {
                        foreach ($optionsOrderProps[$personType] as $key => $orderProp) {
                            if (array_key_exists($key, $order) || $orderProp == 'ADDRESS') {
                                $somePropValue = $propertyCollection->getItemByOrderPropertyId($propsKey[$orderProp]['ID']);
                                if ($key == 'fio') {
                                    self::setProp($somePropValue, $order[$key]);
								}elseif($orderProp == 'ADDRESS'){
			                        $orderCrm = RCrmActions::apiMethod($api, 'orderGet', __METHOD__, $order['id']);

self::setProp($somePropValue, RCrmActions::fromJSON($orderCrm['order']['delivery']['address']['region'].' '.$orderCrm['order']['delivery']['address']['cityType'].
				$orderCrm['order']['delivery']['address']['city'].' '.$orderCrm['order']['delivery']['address']['text']));

									if($orderCrm['order']['delivery']['address']['city']){
try {
$parameters['filter']['PHRASE'] = RCrmActions::fromJSON($orderCrm['order']['delivery']['address']['city']);
$parameters['filter']['NAME.LANGUAGE_ID'] = 'ru';
									...
									if($orderCrm['order']['delivery']['address']['index']){
$somePropValue = $propertyCollection->getItemByOrderPropertyId(4);
                                            self::setProp($somePropValue, $orderCrm['order']['delivery']['address']['index']);
}
								} else {
                    ...
                    if (isset($order['items'])) {
                        ...
                        foreach ($order['items'] as $product) {
							$item = self::getExistsItem($basket, 'catalog', $product['offer']['xmlId']);//['externalId']);
                            ...
                            if (!$item) {
                                ...
                                $item = $basket->createItem('catalog', $product['offer']['xmlId']);//['externalId']);

                                if ($item instanceof \Bitrix\Sale\BasketItem) {
                                    $elem = self::getInfoElement($product['offer']['xmlId']);//['externalId']);
									$rrrrr = CIBlockElement::GetList(($by = 'ID'), ($order = 'DESC'), array('ID' => $product['offer']['xmlId']));
									//mail('dimm4ik@yandex.ru','6666',print_r($elem,true).print_r($rrrrr,true));
                    ...
                    //delivery
                    if (array_key_exists('delivery', $order) || array_key_exists('shipmentStore', $order)) {
                        $itemUpdate = true;
                        //delete empty
                        if (!isset($orderCrm)) {
                            $orderCrm = RCrmActions::apiMethod($api, 'orderGet', __METHOD__, $order['id']);
...
$arr_pickup = array('s11'=>'8','s12'=>'9','s5'=>'4','s10'=>'7','s4'=>'3','s6'=>'5');
    ...
    public static function getInfoElement($offerId)
    {
        $elementInfo = CIBlockElement::GetByID($offerId)->fetch();
			$offer = CIBlockElement::GetList(array(), array("IBLOCK_ID" => 3, "ID" => $offerId), false, false,
                array(
                "ID",
                "IBLOCK_ID",
...
"IBLOCK_EXTERNAL_ID",
		...
		//mail('dimm4ik@yandex.ru','555',print_r($elementInfo,true).print_r($offer,true));
        ...
        $info = array(
			...
			'C_CODE' => $elementInfo["PROPERTY_C_CODE"],
