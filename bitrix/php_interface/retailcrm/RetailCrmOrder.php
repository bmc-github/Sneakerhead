...
class RetailCrmOrder
{
    ...
    {      
		...
		//$arUser = Bitrix\Main\UserTable::getById($arFields['USER_ID'])->fetch();
        $order = array(
		...
		//местоположение
		$arParams['optionsOrderProps'][$arFields['PERSON_TYPE_ID']]['loc'] = 'LOCATION';
...
$arLocs = CSaleLocation::GetByID($prop['VALUE'][0], LANGUAGE_ID);
						...
						$order['delivery']['address']['city'] = $arLocs['CITY_NAME'];
						$order['delivery']['address']['region'] = $arLocs['REGION_NAME'];
					}elseif($prop['CODE'] == 'ADDRESS'){

                    $order['delivery']['address']['street'] = $prop['VALUE'][0];
					}
                }
            }
        }
        ...
        if (array_key_exists($arFields['DELIVERYS'][0]['id'], $arParams['optionsDelivTypes'])) {
			...
			if($arFields['DELIVERYS'][0]['id'] == 3){

					$order['shipmentStore'] = $arFields['DELIVERYS'][0]['pickup'];
				}
			if($arFields['DELIVERYS'][0]['track_number'] && ($arFields['STATUS_ID'] == 'F' || $arFields['STATUS_ID'] == 'OF')){
				$order['delivery']['data']['trackNumber'] = $arFields['DELIVERYS'][0]['track_number'];
				$order['customFields']['track_number'] = $arFields['DELIVERYS'][0]['track_number'];
			}
        }elseif(array_key_exists($arFields['DELIVERYS'][1]['id'], $arParams['optionsDelivTypes'])) {
            $order['delivery']['code'] = $arParams['optionsDelivTypes'][$arFields['DELIVERYS'][1]['id']];
            if (isset($arFields['DELIVERYS'][1]['service']) && $arFields['DELIVERYS'][1]['service'] != '') {
                $order['delivery']['service']['code'] = $arFields['DELIVERYS'][1]['service'];

            }
				if($arFields['DELIVERYS'][1]['id'] == 3){

					$order['shipmentStore'] = $arFields['DELIVERYS'][1]['pickup'];
				}
			if($arFields['DELIVERYS'][1]['track_number'] && ($arFields['STATUS_ID'] == 'F' || $arFields['STATUS_ID'] == 'OF')){
				$order['delivery']['data']['trackNumber'] = $arFields['DELIVERYS'][1]['track_number'];
				$order['customFields']['track_number'] = $arFields['DELIVERYS'][1]['track_number'];
			}
        }

$status = $arFields['STATUS_ID'];
if($status != 'N' && $status !=    'NS' && $status !=  'NZ' && $status !=  'ZO' && $status != 'VR' &&  $status != 'OT'){
        ...
        foreach ($arFields['BASKET'] as $product) {
            ...
            $item = array(
				...
				'offer'           => array(//'externalId' => $product['PRODUCT_ID'],
					'xmlId' => $product['PRODUCT_ID']//$xlmIdCrm['1']//$product['PRODUCT_XML_ID']
                                        ),
    ...
    public static function orderObjToArr($obOrder)
    {
        ...
        foreach ($shipmentList as $shipmentData) {
			...
			if($delivery['ID'] == 3){
				...
				if($shipmentData->getfield('DELIVERY_DOC_NUM')){
					$shipment['track_number'] = $shipmentData->getfield('DELIVERY_DOC_NUM');
				}
