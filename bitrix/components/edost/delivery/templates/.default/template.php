...
if ($mode == 'user_order') {
	...
	foreach ($shipment as $k => $v) {
		$link = (!empty($param['order_detail_link']) ? str_replace(array('%order_id%', '%order_number%'), array($v['order_id'], $v['order_number']), $param['order_detail_link']) : '/personal/order/detail/'.$v['order_number'].'/');
