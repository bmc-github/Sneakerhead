...
if($this->StartResultCache(false, 'v5'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
    ...
    if(!empty($arResult["ITEMS"]))
    {
        if ($this->facet->isValid())
        {
            ...
            if(substr_count($APPLICATION->GetCurPage(false),'isnew')>0)
                $arResult["FACET_FILTER"]["PROPERTY_ISNEW_VALUE"] = 'да';
            if(substr_count($APPLICATION->GetCurPage(false),'sale')>0)
                $arResult["FACET_FILTER"]["PROPERTY_SALE_VALUE"] = 'да';
            ...
            if(substr_count($APPLICATION->GetCurPage(false),'isnew')>0)
                $arElementFilter["PROPERTY_ISNEW_VALUE"] = 'да';
            if(substr_count($APPLICATION->GetCurPage(false),'sale')>0)
                $arElementFilter["PROPERTY_SALE_VALUE"] = 'да';
    ...
    if ($arParams["XML_EXPORT"] === "Y")
    {
        ...
        {
            $arSelect = array("ID", "IBLOCK_ID", "LEFT_MARGIN", "RIGHT_MARGIN", "DEPTH_LEVEL");
...
if ($arParams['SMART_FILTER_PATH']) {
    ...
    $cacheTime = 360000;
    $cacheId = md5($arParams['SECTION_ID'] . $arParams['SMART_FILTER_PATH']);
    $cacheDir = 'bx_filter';
    if ($cache->initCache($cacheTime, $cacheId, $cacheDir)) {
        $FILTER = $cache->getVars();
    } elseif ($cache->startDataCache()) {
        $FILTER['URL'] = explode("/", $arResult['VARIABLES']['SMART_FILTER_PATH']);
        \Bitrix\Main\Loader::includeModule('iblock');

        $arPropsFilter = \CIBlockSectionPropertyLink::GetArray($arParams['IBLOCK_ID'], $arResult['VARIABLES']['SECTION_ID']);
        $arPropsIds = [];
        foreach ($arPropsFilter as $item) {
            if ($item['SMART_FILTER'] === 'Y')
                $arPropsIds[] = $item['PROPERTY_ID'];
        }
        $dbIblockProps = \Bitrix\Iblock\PropertyTable::getList(array(
            'select' => array('*'),
            'filter' => array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ID' => $arPropsIds)
        ));
        while ($arIblockProps = $dbIblockProps->Fetch()){
            $FILTER['PROPS']['PRODUCT'][] = mb_strtolower($arIblockProps['CODE']);
        }
        $mxResult = \CCatalogSKU::GetInfoByProductIBlock($arParams["IBLOCK_ID"]);
        if (is_array($mxResult)) {
            $arPropsFilter = \CIBlockSectionPropertyLink::GetArray($mxResult['IBLOCK_ID'], $arResult['VARIABLES']['SECTION_ID']);
            $arPropsIds = [];
            foreach ($arPropsFilter as $item) {
                if ($item['SMART_FILTER'] === 'Y')
                    $arPropsIds[] = $item['PROPERTY_ID'];
            ...
            $dbIblockProps = \Bitrix\Iblock\PropertyTable::getList(array(
                'select' => array('*'),
                'filter' => array('IBLOCK_ID' => $mxResult['IBLOCK_ID'], 'ID' => $arPropsIds)
            ));
            while ($arIblockProps = $dbIblockProps->Fetch()) {
                $FILTER['PROPS']['OFFER'][] = mb_strtolower($arIblockProps['CODE']);
            }
        }
...
elseif($arParams["SMART_FILTER_PATH"])
    $_CHECK = $this->convertUrlToCheck($arParams["~SMART_FILTER_PATH"], $FILTER['PROPS']);
...
if ($arParams["SEF_MODE"] == "Y")
{
    ...
    $arResult["JS_FILTER_PARAMS"]["SEF_DEL_FILTER_URL"] = str_replace('clear/', "", $this->makeSmartUrl($url, false));
}
...
$arInputNames = array();
$arResult['FILTER_COUNT_CHECKED'] = 0;
foreach($arResult["ITEMS"] as $PID => $arItem)
{
    foreach($arItem["VALUES"] as $key => $ar)
    {
        ...
        if ($ar['CHECKED']) {
            $arResult['FILTER_COUNT_CHECKED'] += 1;
        }
        if ($ar['URL_ID']) {
            $arResult["ITEMS"][$PID]['DISPLAY_TYPE'] = 'URL';
        }
    }
    $arResult['FILTER_URL_PROPS'][$arItem['CODE']] = '';
}
...
$arResult['FILTER_URL'] = explode("/", trim($arResult['FORM_ACTION'], "/"));
$arResult['FILTER_COUNT_PARAMS'] = $arParams['SMART_FILTER_PATH'] ? count(explode("/", $arParams['SMART_FILTER_PATH'])) : 0;
$arFilterSection = [];
foreach ($arResult['FILTER_URL'] as $val) {
    $count = \Bitrix\Iblock\SectionTable::getCount(array(
...
$GLOBALS['arFilterSection'] = $arFilterSection;
$arResult['FILTER_URL_CLEAR'] = $arParams['FOLDER'] . ($arFilterSection ? (implode("/", $arFilterSection) . "/") : '');
$arResult["SEF_DEL_FILTER_URL"] = $arResult['FILTER_URL_CLEAR'];
$arResult["JS_FILTER_PARAMS"]["SEF_DEL_FILTER_URL"] = $arResult['FILTER_URL_CLEAR'];

$params = explode("/", $arParams['SMART_FILTER_PATH']);
if ($params) {
    $count = \Bitrix\Iblock\SectionTable::getCount(array(
        '=IBLOCK_ID' => $arParams['IBLOCK_ID'],
        '=CODE' => $params
    ));
    $arResult['FILTER_COUNT_PARAMS'] -= $count;
}

foreach ($params as $key_url => $val) {
    foreach ($arResult["ITEMS"] as $key_prop => $arItem) {
        foreach ($arItem["VALUES"] as $key_val => $ar) {
            if ($ar['URL_ID']) {
                if ($val == $ar['URL_ID']) {
                    $arResult['FILTER_URL_PROPS'][$arItem['CODE']] = $ar['URL_ID'];
                }
            }
        }
    }
}
if ($arResult['FILTER_COUNT_CHECKED'] != $arResult['FILTER_COUNT_PARAMS']) {
    \Bitrix\Iblock\Component\Tools::process404(
        'Не найден',
        true,
        true,
        \Bitrix\Main\Application::getDocumentRoot()."/404.php"
    );
}
