...
$templateData = array('TEMPLATE_THEME' => $this->GetFolder().'/themes/'.$arParams['TEMPLATE_THEME'].'/colors.css', 'TEMPLATE_CLASS' => 'bx-'.$arParams['TEMPLATE_THEME']);
if(isset($templateData['TEMPLATE_THEME']))
    $this->addExternalCss($templateData['TEMPLATE_THEME']);
?>
<div class="bx-filter <?= $templateData["TEMPLATE_CLASS"] ?> box" id="filterpro_box">
    <div class="bx-filter-section container-fluid box-content filterpro">
        <form id="filterpro" name="<?=$arResult["FILTER_NAME"]."_form" ?>" action="<?=$arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
            ...
            <?if(!empty($arResult['CATEGORIES'])){
              ...
              <div class="col-lg-12 bx-filter-parameters-box bx-active filterRow">
	        <span class="bx-filter-container-modef"></span>
                <div class="bx-filter-parameters-box-title filterTitle" onclick="smartFilter.hideFilterProps(this)">
                  <span class="bx-filter-parameters-box-hint"><?if($_SESSION['lang'] == null || $_SESSION['lang'] == 'ru') echo $parent['NAME']; else echo $parent['UF_NAME']; ?> <i data-role="prop_angle" class="fa fa-angle-up"></i></span>
                </div>
		<div class="bx-filter-block filterContainer" data-role="bx_filter_block">
                  <div class="row bx-filter-parameters-box-container">
		  <?foreach($arResult['CATEGORIES'] as $it){
                      if($it['IBLOCK_SECTION_ID'] == $parent['ID']){?>
                    <div class="col-xs-12">
                      <div class="checkbox">
		        <a href="<?=$it['SECTION_PAGE_URL']?>"<?if(substr_count($_SERVER['REQUEST_URI'],$it['SECTION_PAGE_URL']) > 0) echo ' class="activeLink"'; ?>><?=$it[(($_SESSION['lang'] == 'en') ? 'UF_' : '').'NAME'] ?></a>
                      </div>
                    </div>
		  <?  }
                    }?>
...
foreach($arResult["ITEMS"] as $key => $arItem){
  ...
  if($arItem['CODE'] == 'SIZES_SHOES')
    echo '<!--noindex-->'; ?>
                    <div class="col-lg-12 bx-filter-parameters-box <?if($arItem["DISPLAY_EXPANDED"] == "Y"):?>bx-active<?endif?> filterRow">
                        <span class="bx-filter-container-modef"></span>
                        <div class="bx-filter-parameters-box-title filterTitle" onclick="smartFilter.hideFilterProps(this)">
			    <span class="bx-filter-parameters-box-hint"><?if($_SESSION['lang'] == null || $_SESSION['lang'] == 'ru') echo $arItem['NAME']; else echo (($arItem['CODE'] == 'SIZES_SHOES') ? 'SIZE' : $arItem['CODE']);?>
...
?>
						...
						<span class="bx-filter-input-checkbox">
							...
							<span class="bx-filter-input-checkbox <? echo $ar["DISABLED"] ?'disabled' : '' ?>">
								...
								<span class="bx-filter-param-text" title="<?= $ar["VALUE"]; ?>"><?= $ar["VALUE"]; ?><?if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):?>&nbsp;(<span data-role="count_<?= $ar["CONTROL_ID"] ?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)<?endif; ?></span>
							</span>
                                                </label>
                                            </div>
                                        <? endforeach; ?>
            ...
            default: //CHECKBOXES
                foreach ($arItem["VALUES"] as $val => $ar):?>
                                            <div class="checkbox">
                                                ...
                                                <span class="bx-filter-input-checkbox">
                                                    <input type="checkbox" value="<?=$ar["HTML_VALUE"]?>" name="<?=$ar["CONTROL_NAME"]?>" id="<?=$ar["CONTROL_ID"]?>" <? echo $ar["CHECKED"] ? 'checked="checked"' : '' ?> onclick="smartFilter.click(this)" />
                                                    <span class="bx-filter-param-text" title="<?= $ar["VALUE"]; ?>"><?= $ar["VALUE"]; ?><?if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])): ?>
                ...
                <div class="col-xs-12 bx-filter-button-box">
                    <div class="bx-filter-block">
                        <div class="bx-filter-parameters-box-container">
                            ...
                            <input class="clear_filter" type="submit" id="del_filter" name="del_filter" value="<?= GetMessage("CT_BCSF_DEL_FILTER") ?>" />
                            <div class="bx-filter-popup-result <? if ($arParams["FILTER_VIEW_MODE"] == "VERTICAL") echo $arParams["POPUP_POSITION"] ?>" id="modef" style="display:<?=(!isset($arResult["ELEMENT_COUNT"]))?'none':'inline-block';?>">
...
<script>
  ...
  $('.filter_selectric').selectric({
    ...
    onChange: function(element){
				 ...
				 * $(element).change().val()
				 * */
				//filterCountTop($('.selectric'));
    }
  });
  $(".selectric-items li").click(function(){
    window.location.href = $("#shopList option").get($(this).attr("data-index")).value;
  });
</script>